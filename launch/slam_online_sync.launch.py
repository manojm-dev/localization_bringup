
import os

from launch import LaunchDescription
from launch.actions import DeclareLaunchArgument, IncludeLaunchDescription
from launch.substitutions import LaunchConfiguration
from launch.conditions import IfCondition, UnlessCondition
from launch.launch_description_sources import PythonLaunchDescriptionSource

from launch_ros.actions import Node
from launch_ros.substitutions import FindPackageShare


def generate_launch_description():

    # Packages share directories
    pkg_share = FindPackageShare('localization_bringup').find('localization_bringup')
    pkg_slamtb_share = FindPackageShare('slam_toolbox').find('slam_toolbox')

    # File paths
    slam_config_mapping = os.path.join(pkg_share, 'config/mapper_params_online_sync_mapping.yaml')
    slam_config_localization = os.path.join(pkg_share, 'config/mapper_params_online_sync_localization.yaml')

    # Launch configuration 
    use_sim_time = LaunchConfiguration('use_sim_time')
    localization = LaunchConfiguration('localization')

    # Launch Arguments 
    declare_arguments = [
        DeclareLaunchArgument(
            name='use_sim_time',
            default_value='true',
            choices=['true', 'false'],
            description='Use Simulation(Gazebo) Clock'
        ),
        
        DeclareLaunchArgument(
            name='localization',
            default_value='false',
            choices=['true', 'false'],
            description='Localization mode'
        ),
    ]

    start_slamtb_mapping = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(pkg_slamtb_share, 'launch', 'online_sync_launch.py')), 
        launch_arguments={
            'slam_params_file': slam_config_mapping,
            'use_sim_time': use_sim_time
            }.items(),
        condition=UnlessCondition(localization)
    )
    
    start_slamtb_localization = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(pkg_slamtb_share, 'launch', 'online_sync_launch.py')), 
        launch_arguments={
            'slam_params_file': slam_config_localization,
            'use_sim_time': use_sim_time
            }.items(),
        condition=IfCondition(localization)
    )


    return LaunchDescription(
        declare_arguments + [
            start_slamtb_mapping,
            start_slamtb_localization
        ]
    )
